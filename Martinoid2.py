...
name = "".join([f"N{x.replace('0', '').lower()}-" for x in resid.split("-")])[:-1]
print("Name:", name)



SS = {"straight":{"Angle_deg":161, "Angle_FC":25,                  ## is this semi redundant? 
...
for seq_i, residue in enumerate(seq):
    ...
    if np.where(beads["i"]=="BB")[0].shape[0] >= 2:  
        ...
        bonds.loc[bonds.index.shape[0]] = [bond[0], bond[1], 
                                           ParseData(previous_BB_residue, "BB")[0],
                                           ParseData(previous_BB_residue, "BB")[1]]
        #bonds.loc[bonds.index.shape[0]] = [bond[0], bond[1], res_bb[s-1][0], res_bb[s-1][1]]             ## need to make this residue specific


     #Make BB-BB-BB Angle
    if np.where(beads["i"]=="BB")[0].shape[0] >= 3:
        BB = np.where(beads["i"]=="BB")[0][-3:]
        #angles.loc[angles.index.shape[0]] = [BB[0], BB[1], BB[2], SS["Angle_deg"], SS["Angle_FC"], "BB-BB-BB_"+SS_type] # remove SS_type ? 

        
    #Make BB-BB-BB-BB dihedral
    if np.where(beads["i"]=="BB")[0].shape[0] >= 4:
        BB = np.where(beads["i"]=="BB")[0][-4:]
        #dihedrals.loc[angles.index.shape[0]] = [BB[0], BB[1], BB[2], BB[3], SS["Dihedral_deg"], SS["Dihedral_FC"], "BB-BB-BB-BB_"+SS_type]  # remove SS_type ? 

    # make side-chain  bonds
    new_bonds = ParseData(residue, "bonds")                                  ## now getting BB - BB bond this way 
    ...
    if beads[beads["i"] == "BB"].shape[0] >= 2:
        ...
        SC1 = new_angles.loc["BB-BB-SC1"]["j"]                # was "BB-BB-SC1"
...
#print("Setting N-ter and C-ter to charged")                        ## make this controllable with question or logic test (specifically to weed out unstable species)
...
traj.save_pdb(f"{name}.pdb")                 ## specific sequence name
...
itp = open(f"{name}.itp", 'w')         ## specific sequence name 
NAME = f"{name}"                     ##             "
